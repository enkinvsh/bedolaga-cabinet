...
export default function CurrentPlanModal({ subscription, onClose }: CurrentPlanModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-end justify-center">
      <div 
        className={`absolute inset-0 bg-slate-900/20 backdrop-blur-sm transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
        onClick={handleClose}
      />
      
      <div
        className={`w-full max-w-[430px] bg-white/95 dark:bg-zen-card backdrop-blur-md rounded-t-[2.5rem] p-8 transform transition-transform duration-300 relative ${
      ...
      >
        ...
        <h3 className="text-2xl font-bold text-zen-text mb-2">
          {subscription.tariff_name || t('zen.plan.currentPlan', 'Current Plan')}
        </h3>
